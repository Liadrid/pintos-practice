...
thread_unblock (struct thread *t) 
{
  ...
  list_insert_ordered (&ready_list, &t->elem, &thread_cmp_priority, t->priority);
...
thread_yield (void) 
{
  ...
  old_level = intr_disable ();
  if (cur != idle_thread) 
    list_insert_ordered (&ready_list, &cur->elem, &thread_cmp_priority, cur->priority);
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  old_level = intr_disable ();
  list_insert_ordered (&all_list, &t->allelem, &thread_cmp_priority, t->priority);
  intr_set_level (old_level);
}
